<style media="screen">
    ...
    .container-calendar .content-calendar {
        background: #f0f0f0;
        border-radius: 4px;
        box-shadow: 2px 2px 8px 2px rgba(80,80,80,.35);
        display: none;
        overflow: hidden;
        padding: 0px 4px;
        position: absolute;
    ...
    .container-calendar .content-calendar.active {
        display: block;
        min-width: 656px;
    }

    /* Filtro fechas */
    /* .button-radio-full-screen {
        box-shadow: none;
        cursor: pointer;
        position: absolute;
        right: 8px;
    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header card-header-danger p-0">
                <div class="row">
                    <div class="col col-8">
                        <h4 class="card-title p-15">Historial De Ventas</h4>
                    </div>
...
<script>
    ...
    ctrlData.requestInvoices = <?= json_encode($requestInvoices) ?>;
</script>
<?= $this->Html->script('historial_ventas.js', ['block' => 'scriptBottom']) ?>
