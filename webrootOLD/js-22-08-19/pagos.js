    ...
    $(document).on('change', 'input.chk-credi', function(evt) {
        ...
        if (chk.prop('checked')) {
            ...
            if (data_pago[0].reg == 1 && data_pago[0].dia_mora == 0) {
                swal({
                    title: 'Información!',
                    text: 'Cliente con derecho a CREDI-DESCUENTO, preguntar al cliente si paga la totalidad',
                    icon: 'info',
                    dangerMode: true
                }).then((value) => {});
            }
...
function fillTable(data) {
    ...
    for (obj of data) {
        ...
        var classD1 = '';
        var proc = '';
        var procIco = '<input type="checkbox" tabindex="-1" class="chk-credi" data-doc="'+obj.num_doc+'" />';
        var inputPayment = '<input type="text" class="pagoTabla" onkeypress="return isNumber(event)" onkeyup="inputPay(event);formatNumber(event,\'\',true);" data-doc="'+obj.num_doc+'" />';
...
function showModal() {
    ...
    $('.cont-modal').fadeIn();
    var data_pago = _.map($('input.chk-credi:checked'), function(o) { return {"factura": $(o).data("doc"), "abono": $('input.pagoTabla[data-doc="'+$(o).data("doc")+'"]').val().replace(/\,/g, ''), "cuotas": _.where(_dataPago[2], {num_doc: $(o).data("doc")}) } });
    var items = '';
    var i = 0;
    var subtotal = 0;
    var descuentos = 0;
    ...
    for (dp of data_pago) {
        ...
        for(cuota of dp.cuotas) {
            ...
            var descuento = cuota.s_finan + cuota.s_aval;
            if (docDiff == 0) {
